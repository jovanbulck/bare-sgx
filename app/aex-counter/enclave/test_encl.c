...
static void ud_handler(struct ssa_frame *frame)
{
	...
	if (ip_opcode == CPUID_OPCODE)
    {
		/*
		 * TODO: simplify
		 * dont actually need this logic here to demonstrate it can be used
		 * such as the if statement below, but this adapted from
		 * https://github.com/intel/intel-sgx-ssl/blob/main/Linux/sgx/libsgx_tsgxssl/texceptions.c
		 */
